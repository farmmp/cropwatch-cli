...
from cropwatch.cli_zscore import zscore_group
...
@click.group()
def cli() -> None:
    """CropWatch — USDA crop progress from your terminal."""


@cli.command()
@click.option("--commodity", "-c", default="CORN", show_default=True)
@click.option("--year", "-y", default=2023, show_default=True, type=int)
@click.option("--state", "-s", default=None)
@click.option("--simple", is_flag=True, default=False, help="Plain table output.")
@click.option("--export", type=click.Choice(["json", "csv"]), default=None)
...
def history_cmd(limit: int) -> None:
    ...
    for e in reversed(entries):
        ...
        click.echo(f"{e.get('timestamp', '?')}  {e.get('commodity','?')} {e.get('year','?')}{state_part}")
...
cli.add_command(leadlag_group, name="leadlag")
cli.add_command(zscore_group, name="zscore")
cli.add_command(dominance_group, name="dominance")
cli.add_command(concentration_group, name="concentration")
